...
export function getArbitraryFutureTimestamp() {
    ...
    const oneHourInMilliseconds = 3600000; // 60 minutes * 60 seconds * 1000 milliseconds
    const futureTimestampUnix = currentTimestamp + oneHourInMilliseconds;

    return { timestamptz: futureTimestamp, unix: futureTimestampUnix };
}

// Function: Get last stored Gmail message ID from Supabase gMailMessages table
export async function getLastStoredGMsgId(emailAccountId) {
    ...
    try {
        var { data, error, status, statusText } = await supabaseClient
            .from("gmailMessages")
            .select("emailAccountId, gMsgId, receivedAt")
...
export async function getLastGHistoryId(emailAccountId) {
    ...
    try {
        const { data, error, status, statusText } = await supabaseClient
            .from("emailAccounts")
            .select("userId, id, lastGmailHistoryId, syncedAt")
            .eq("id", emailAccountId)
            .lt("createdAt", futureTimestamp.timestamptz);  
...
export async function isGmailMsgStored(supabaseClient, emailAccountId, gMsgId) {
    ...
    try {
        const { data, error, status, statusText } = await supabaseClient
            ...
            .eq("gMsgId", gMsgId)
            .eq("emailAccountId", emailAccountId)
            .lt("createdAt", futureTimestamp.unix);   
