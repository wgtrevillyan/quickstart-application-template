  ...
  async run({ authId, emailAccountId, messages }) {
    try {
      ...
      const newSenderAddresses = senderAddresses.filter((senderAddress) => {
        return storedSenderAddresses.every(
          (storedSenderAddress) =>
            storedSenderAddress.senderAddress !== senderAddress
        );
      });
      console.log(
        `Found ${newSenderAddresses.length} new sender addresses to store.`
      );

      // Remove duplicates from the new sender addresses.
      const uniqueNewSenderAddresses = [...new Set(newSenderAddresses)];
      console.log(
        `Found ${uniqueNewSenderAddresses.length} unique sender addresses to store.\n`
      );
      ...
      for (const newSenderAddress of uniqueNewSenderAddresses) {
        try {
          i++;
          console.log(
            `Inserting sender address ${i} of ${uniqueNewSenderAddresses.length} into the senderAddresses table...\r`
          );
          ...
          if (error) {
            throw new Error(
              `Unexpected Error occured when inserting message ${i}: ${status} ${statusText} - ${error.message}`
            );
          } else if (status !== 201) {
            throw new Error(
              `Error storing sender address: ${status} ${statusText}`
            );
          } else if (!data) {
      ...
      if (errorMessages.length > 0) {
        console.error(
          `${errorMessages.length} errors occurred while inserting sender addresses into the table:`
        );
      ...
      console.error(
        "An error occurred while storing sender addresses in Supabase:",
        error
      );
